...
// Types
export type Person = components['schemas']['suasor_clients_media_types.Person'];
...
export const personService = {
	...
	updatePerson: async (id: string, request: UpdatePersonRequest): Promise<Person> => {
		const response = await PUT(`/people/{personID}`, {
			params: { path: { id } },
			body: request
		});
		return response.data?.data;
	...
	getPersonCredits: async (personId: string): Promise<Credit[]> => {
		const response = await GET(`/people/{personID}/credits`, {
			params: { path: { personID: personId } }
		});
		return response.data?.data || [];
	},

	getMediaItemCredits: async (itemId: string): Promise<Credit[]> => {
		const response = await GET(`/item/{itemID}/credits`, {
	...
	createMultipleCredits: async (
		mediaItemId: string,
		request: CreateCreditsRequest
	): Promise<Credit[]> => {
