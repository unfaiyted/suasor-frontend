<script lang="ts">
  ...
  let isMinimized = $state<boolean>(true); // Default to minimized
  let isHidden = $state<boolean>(false); // Whether the component is completely hidden
...
<!-- Main container with conditional classes based on minimized and hidden states -->
{#if !isHidden}
  <div 
        ...
        {#if showDetails}
          <div class="flex flex-col mt-1 pt-1 border-t border-gray-700">
            ...
            <div class="text-gray-400 text-[10px] mt-1">
              Double-click to minimize, click +/- to toggle details, Ctrl+Shift+A to hide/show completely
            </div>
          </div>
        {/if}
      </div>
