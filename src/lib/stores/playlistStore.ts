...
import type { MediaItemPlaylist } from '$lib/api/types';
import { mediaItemApi } from './mediaItem';

import { TypesMediaType } from '$lib/api/suasor.v1.d';
	...
	async addItemToPlaylist(playlistId: string, mediaItemId: string, order?: number) {
		...
		try {
			...
			if (actualOrder === undefined) {
				...
				actualOrder =
					currentItems.length > 0 ? Math.max(...currentItems.map((item) => item.order)) + 1 : 0;
			}
...
export const playlists = derived(playlistStore, ($state) => Object.values($state.playlists));
...
export function getPlaylistItems(playlistId: string) {
	return derived(playlistStore, ($state) => $state.playlistItems[playlistId] || []);
}
