...
export const mediaItemApi = {
  ...
  async searchMediaItems(query: string, filters?: Partial<MediaItemFilters>) {
      ...
      mediaItemStore.update((state) => ({
        ...
        mediaItems: {
          ...
          ...(combinedFilters.mediaType 
            ...
            : Object.entries(cachedData.itemsByType).reduce((acc, [type, items]) => ({
                ...acc,
                [type]: { ...state.mediaItems[type as keyof MediaItemState['mediaItems']], ...items }
              }), {})
          )
        }
      }));
    ...
    try {
      ...
      let endpoint;
      if (combinedFilters.mediaType === 'album' || 
      ...
      mediaItemStore.update((state) => ({
        ...
        mediaItems: {
          ...
          ...(combinedFilters.mediaType
            ...
            : Object.entries(itemsByType).reduce((acc, [type, items]) => ({
                ...acc,
                [type]: { ...state.mediaItems[type as keyof MediaItemState['mediaItems']], ...items }
              }), {})
          ),
  ...
  async addMediaItem(mediaItem: AddMediaRequest) {
    ...
    try {
      ...
      mediaItemCache.invalidatePattern(new RegExp(`^mediaItem:.*:${state.activeMediaClientId}:${newItem.mediaType}`));
  ...
  async updateMediaItem(id: string, mediaType: MediaType, updates: UpdateMediaRequest) {
    ...
    try {
      ...
      // Invalidate relevant caches
      mediaItemCache.invalidate(`mediaItem:details:${state.activeMediaClientId}:${mediaType}:${id}`);
      mediaItemCache.invalidatePattern(new RegExp(`^mediaItem:.*:${state.activeMediaClientId}:${mediaType}`));
...
export const albums = derived(mediaItemStore, ($state) => 
  Object.values($state.mediaItems.album)
);

export const artists = derived(mediaItemStore, ($state) => 
  Object.values($state.mediaItems.artist)
);

export const tracks = derived(mediaItemStore, ($state) => 
  Object.values($state.mediaItems.track)
);

export const movies = derived(mediaItemStore, ($state) => 
  Object.values($state.mediaItems.movie)
);

export const series = derived(mediaItemStore, ($state) => 
  Object.values($state.mediaItems.series)
);
