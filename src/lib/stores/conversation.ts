		...
		convertRecommendationsToMovies(recommendations: any[]): Movie[] {
			...
			return recommendations.map((rec: any, index: number) => {
				...
				return {
					...
					poster:
						artwork.poster ||
						`https://via.placeholder.com/300x450.png?text=${encodeURIComponent(details.title || 'Movie')}`,
					backdrop: artwork.background,
					genres: details.genres || [],
					rating:
						typeof details.userRating === 'number'
							? details.userRating
							: details.Ratings?.imdb || 0,
